      ...
      <div className="relative w-full max-w-lg bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-5 pb-8 space-y-6 pt-4">
          ...
          <div>
            ...
            {/* Built-in badge */}
            <div className="bg-green-50 border border-green-100 rounded-xl px-3 py-2.5 mb-3 flex items-start gap-2">
              <span className="text-base shrink-0">✅</span>
              <div>
                <p className="text-xs font-semibold text-green-700">Built-in AI — free, no key needed</p>
                <p className="text-xs text-stone-500 mt-0.5">Everyone who uses this app gets AI automatically. Powered by Groq (Llama 3.3 70B).</p>
              </div>
            </div>

            <p className="text-xs font-medium text-stone-600 mb-1">Use your own key <span className="font-normal text-stone-400">(optional — for more control or a smarter model)</span></p>
            <p className="text-xs text-stone-400 mb-2">
              OpenRouter (sk-or-…) · Groq (gsk_…) · Gemini (AIza… — has strict free limits)
            </p>
            ...
            <p className="text-xs text-green-600 mt-1">
              {settings.geminiApiKey
                ? `✓ Using your key · ${settings.geminiApiKey.startsWith('gsk_') ? '⚡ Groq' : settings.geminiApiKey.startsWith('sk-') ? '🔀 OpenRouter' : '✨ Gemini'}`
                : '✓ Using built-in AI'}
            </p>
          </div>
